...
def _stub_predictor(standalone: bool) -> Qwen3_5MultiTokenPredictor:
    ...
    # The real Qwen3_5DecoderLayer always exposes
    # `use_attn_reduce_scatter_for_moe` (qwen3_5.py); the fork's forward reads
    # it before/after the layer call, so the stub must carry the attribute.
    layer = lambda positions, hidden_states, residual: (hidden_states, residual)
