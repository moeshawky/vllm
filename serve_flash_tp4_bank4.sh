...
export VLLM_XLA_CACHE_PATH=/dev/shm/vllm_xla_cache_flash_tp4_bank4
...
mkdir -p /dev/shm/hf /dev/shm/vllm_xla_cache_flash_tp4_bank4
MODEL=/kaggle/input/models/manarmilad/qwen3.8-flash-next-uncensored/transformers/fp8/1
echo "[flash_tp4_bank4] $(date) start wall=$(date +%s) HEAD=$(git -C /kaggle/working/vllm-src rev-parse HEAD) tpu-inf=$(git -C /kaggle/working/tpu-inference rev-parse HEAD)"
echo "[bank] count $(ls $QWEN_BANK_DIR/layer_*.bank 2>/dev/null | wc -l) at $QWEN_BANK_DIR"
echo "[patch] common mix fix applied: $(grep -n "block_input, injection = attn_hc.mix" /kaggle/working/vllm-src/vllm/models/qwen4_exp/common/model.py)"
